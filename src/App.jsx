...
function GestionGrupos({ onVolver }) {
  ...
  const iniciarEdicion = (g) => {
    setEditando(g); setNombre(g.nombre); setHorario(g.horario || '')
    window.scrollTo(0, 0)
  }
  ...
  return (
    <div style={{ maxWidth:'520px', margin:'0 auto', padding:'16px' }}>
          ...
          {confirmEliminar === g.id ? (
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              <div style={{ display:'flex', gap:'6px' }}>
                ...
                <button onClick={() => setConfirmEliminar(g.id)}
                  style={{ padding:'7px 12px', background:'#fee2e2', color: G.rojo, border:'none', borderRadius:'7px', cursor:'pointer', fontSize:'13px' }}>🗑️</button>
              </div>
            </div>
          )}
        </div>
      ...
      {grupos.length === 0 && (
        <p style={{ textAlign:'center', color: G.gris, marginTop:'32px', fontSize:'14px' }}>No hay grupos aún.</p>
      )}
    </div>
  )
}
    ...
    <div style={{ maxWidth:'520px', margin:'0 auto' }}>
      ...
      <div style={{ background:'white', position:'sticky', top:'52px', zIndex:90, borderBottom:`1px solid ${G.borde}` }}>
        <div style={{ display:'flex', overflowX:'auto', scrollbarWidth:'none' }}>
          ...
          <button onClick={() => setVerGestionGrupos(true)}
            style={{ flexShrink:0, padding:'13px 14px', border:'none', background:'transparent',
              color: G.gris, cursor:'pointer', fontSize:'18px', borderBottom:'3px solid transparent' }}>
            ⚙️
          </button>
        </div>
      </div>
      ...
      <div style={{ padding:'16px' }}>
        {grupos.length === 0 ? (
          <div style={{ textAlign:'center', marginTop:'60px' }}>
            ...
            <button onClick={() => setVerGestionGrupos(true)}
              style={{ padding:'12px 24px', background: G.cafe, color:'white', border:'none', borderRadius:'10px', cursor:'pointer', fontWeight:'bold' }}>
              Crear primer grupo
            </button>
          </div>
        ) : (
                ...
                <div style={{ display:'flex', gap:'6px', marginBottom:'12px', flexWrap:'wrap' }}>
                  {grupos.map(g => (
                    <button key={g.id} type="button" translate="no"
                      onClick={() => { setGrupoId(g.id); setSubgrupo('') }}
                      style={{ padding:'7px 12px', borderRadius:'8px',
                        border:`2px solid ${grupoId === g.id ? G.cafe : G.borde}`,
                        background: grupoId === g.id ? G.cafe : 'white',
                        color: grupoId === g.id ? 'white' : G.gris,
                        cursor:'pointer', fontSize:'13px', fontWeight: grupoId === g.id ? 'bold' : 'normal' }}>
                      {g.nombre}
                    </button>
                  ))}
                </div>
